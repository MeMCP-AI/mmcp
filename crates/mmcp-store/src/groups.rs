...
impl GroupIndex {
    ...
    pub async fn build(
        ...
        std::fs::create_dir_all(&repos_root).map_err(|source| {
            StoreError::io(repos_root.clone(), FileOperation::CreateDir, source)
        })?;
        let index = Self {
            repos_root,
            backend,
    ...
    ///
    /// Mirrors [`Self::try_scope_of`].
    /// The sync engine's `GroupHandleResolver::iter_group_ids` calls this from an async worker.
