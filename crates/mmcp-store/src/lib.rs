...
//!
//! Owns the local-first read/write/sync/diagnose logic every mmcp consumer needs:
//! the `mmcp` CLI binary, the MCP stdio tool surface it serves,
//! the desktop `mmcp-gui` application, and any third-party Rust code driving the store programmatically.
//!
//! ## Modules
//!
...
//! - `lock`: hierarchical scoped lock registry.
//! - `sessions`: per-session state storage.
//! - `import_adoc`: AsciiDoc-to-Markdown conversion for bulk imports.
//! - `error`: one `StoreError` enum covering every failure the store can surface.
//! - `testing` (feature-gated): tempdir-backed fixtures shared across consumer crates' integration tests.
//!
//! ## Consumer contract
//!
//! The crate carries zero dependencies on `rmcp`, `clap`, `inquire`, or `egui`.
//! The public surface is typed structs, `thiserror` errors, and `tokio` async methods.
//! Each consumer owns its own argument parsing, user prompting, and response serialization;
//! the store owns correctness of the underlying git writes, index coherence, and error shapes.
