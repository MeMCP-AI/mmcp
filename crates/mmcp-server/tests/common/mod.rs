...
//! `use`) in each one that needs it. Centralizes the 9-field
...
/// `mmcp_auth` password-length defaults. `repo_root` is the one
/// field every caller must genuinely supply (each test uses its own
/// tempdir); `token_key` and `oauth_providers` are the two fields
/// that vary per test file, and callers override them with struct-
/// update syntax, e.g. `ServerConfig { token_key: [7u8; 32],
