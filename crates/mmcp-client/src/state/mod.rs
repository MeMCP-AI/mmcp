...
//!
//! `SessionStore`, `GroupIndex`, and `StateError` live in
//! `mmcp-store` so the desktop GUI and any third-party consumer
//! can drive the stores without depending on the CLI binary.
//!
//! What remains here is strictly client-specific plumbing that
//! `mmcp-store` does not own:
//!
