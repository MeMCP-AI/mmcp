    ...
    report
        ...
        .map(|(remote_name, p)| {
            ...
            Note::warn(
                ...
                format!(
                    "push for group {} via remote '{remote_name}' did not ship content ({reason})",
                    p.group_id
                ),
            )
            .with_context(json!({
...
mod tests {
    ...
    fn sync_push_partial_failure_notes_uses_the_real_transport_error_when_present() {
        ...
        assert!(
            notes[0].message.contains("Permission denied (publickey)"),
            "note message must carry the real transport error, got: {}",
            notes[0].message
        );
        assert_eq!(
            notes[0]
                .context
                .as_ref()
                .and_then(|c| c.get("transport_error"))
                .and_then(|v| v.as_str()),
