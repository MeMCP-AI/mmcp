...
use crate::commands::tool_metadata_cli::{
...
///
/// Kept as a separate enum so the JSON schema exported by `rmcp` is owned by this crate.
/// The mmcp-core definition stays serde-only and unaware of schemars.
/// `rename_all = "snake_case"` matches [`mmcp_core::manifest::GroupScope`]'s own serde rename.
/// The wire strings are identical: `"global"`, `"shared"`, `"project"`.
#[derive(Debug, Clone, Copy, Deserialize, JsonSchema)]
#[schemars(crate = "rmcp::schemars")]
#[serde(rename_all = "snake_case")]
enum ToolGroupScope {
...
/// Render a [`mmcp_core::manifest::GroupScope`] as its wire string.
/// Matches the enum's serde `rename_all = "snake_case"`.
/// This keeps response and request vocabularies in lockstep.
...
impl McpServer {
    ...
    async fn subscribe(
        &self,
        Parameters(args): Parameters<SubscribeMcpArgs>,
    ) -> Result<CallToolResult, McpError> {
        self.apply_subscription_mcp(args, SubscriptionAction::Subscribe)
            .await
    }
    ...
    async fn update_feature(
        ...
        let superseded_by = match args.superseded_by {
            None => None,
            Some(arg) => Some(parse_single_wire_ref(arg, "superseded_by")?),
        };
...
impl McpServer {
    ...
    /// `mmcp tools` CLI, and the `diagnose` annotation-coverage
    /// check. Derived by cloning `Self::tool_router().map`, the same
    /// map `#[tool_router]` builds from every `#[tool]` site, sorted
    /// by name for a deterministic catalogue: a `#[tool]` method can
    /// no longer go missing from this list, or linger here after
    /// removal, because there is no second hand-written list to
    /// drift from it.
    /// `tool_annotations_match_fr029_matrix` separately hard-codes
    /// each named tool's expected annotation bits and does not walk
    /// this list.
...
/// category-derived icons, `_meta`, and an output schema so
...
pub(crate) fn registered_tool_attrs() -> Vec<rmcp::model::Tool> {
    decorate_tool_attrs(McpServer::registered_tool_attrs())
}
...
/// Parse the wire form of [`mmcp_core::memory::FeatureStatus`] from an optional string argument.
/// `None` maps to `Ok(None)`.
/// A known variant maps to `Ok(Some(...))`.
/// An unknown variant maps to a structured `invalid_feature_status` error.
fn parse_status_arg(
    raw: Option<&str>,
...
/// Serialize a [`mmcp_store::features::FeatureRecord`] to the JSON shape returned by the FR tools.
/// Includes the group id so callers can cross-reference without a second `status` call.
...
/// Serialize a [`mmcp_store::features::FeatureSummary`] to the body-free JSON shape.
/// Used by `list_features`.
/// Same fields as `feature_record_to_json` minus `body`.
/// Listings stay metadata-only so populated FR groups don't blow past the MCP client token cap.
...
/// Parse a single-value wire ref field (`superseded_by` on both the
/// feature and issue `update` tools) into its validated
/// [`mmcp_core::memory::MemoryRef`].
///
/// Wraps [`parse_wire_refs`] with the single-input invariant
/// `mmcp_store::parse_memory_refs` always preserves (one input in,
/// one entry out on success): both call sites used to repeat this
/// wrapping inline with a byte-identical `.expect(...)`, one owner
/// here removes the duplication.
fn parse_single_wire_ref(
    arg: MemoryRefArg,
    field: &'static str,
...
/// Map a [`mmcp_store::features::FeatureError`] onto an [`McpError`] with a structured `code`.
/// This lets AI callers branch on state rather than parsing strings.
/// Covers the FR-specific cases first.
/// Then delegates to `map_memory_error_to_mcp` for the wrapped memory CRUD failures.
/// This keeps the wire contract identical between FR tools and the generic memory tools.
/// Those are `write_memory`, `edit_memory`, and `delete_memory`.
...
/// Parse the wire form of [`mmcp_core::memory::IssueStatus`] from an optional string argument.
/// Mirrors `parse_status_arg` for the issue tracker.
...
/// Serialize an [`mmcp_store::issues::IssueRecord`] to the JSON shape the issue tools return.
/// Mirrors `feature_record_to_json`.
...
/// Serialize an [`mmcp_store::issues::IssueSummary`] to the body-free JSON shape.
/// Used by `list_issues`.
/// Mirrors `feature_summary_to_json`.
...
/// Map a [`crate::commands::init::InitProjectError`] to an [`McpError`] with a structured `code`.
/// AI callers can branch on state instead of parsing the error string.
...
/// Map a [`SubscribeError`] to an [`McpError`] with a structured
/// `code` payload.
fn map_subscribe_error_to_mcp(err: SubscribeError) -> McpError {
    let message = err.to_string();
    let payload = match &err {
...
/// Map a [`crate::commands::group::CreateGroupError`] to an [`McpError`] with a structured `code`.
/// AI callers can branch on state instead of parsing the error string.
...
mod tests {
    ...
    async fn read_memory_returns_the_whole_body_even_when_oversized() {
        ...
        // The body is seeded straight through the backend, bypassing the write-time check.
        // Even far above the write-time result ceiling, it still reads back byte-for-byte.
        let (state, _tmp) = test_state().await;
    ...
    async fn subscribe_tag_round_trips_through_config() {
        ...
        use crate::commands::subscription::{SubscribeMcpArgs, SubscriptionKind};
    ...
    /// Catches a regression where the patching step is skipped, or the helper returns an empty Vec.
    /// `every_registered_tool_has_declared_metadata` covers a tool with no metadata entry at all.
    #[test]
    ...
    /// Every live `#[tool]`-registered tool resolves through
    /// `tool_metadata_for_name` without panicking.
    /// Fails loudly (via that panic) the moment a tool ships with no
    /// matching `McpToolId` variant or `tool_metadata` arm, closing
    /// the gap where icon, meta, and risk hints used to default
    /// silently on a missed edit.
    #[test]
    fn every_registered_tool_has_declared_metadata() {
        for tool in registered_tool_attrs() {
            let _ = tool_icon_category(tool.name.as_ref());
        }
    }
