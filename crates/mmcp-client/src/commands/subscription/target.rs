//! Subscription target validation and mutation.
//!
//! Shared by the `subscribe` / `unsubscribe` MCP tools
//! (`commands::serve`) and the `mmcp subscribe` / `mmcp unsubscribe`
//! CLI subcommands (`commands::subscribe`): both layers validate a
//! target against the local mirror, then call the same pure mutator.
//!
