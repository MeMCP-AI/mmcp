<script lang="ts">
  ...
  // Edit / Preview tab — single source of truth across all window
  // widths. Replaces the previous split-view so create / edit feel
  // symmetric with the read-mode viewer.
...
<section class="flex h-full min-h-0 flex-col overflow-hidden bg-zinc-950">
  <!-- Header: title + tabs. Never scrolls. -->
  <header
  ...
  >
    ...
    <div class="ml-auto flex items-center gap-1">
      <button type="button" class={tabBtn(tab === 'edit')} onclick={() => (tab = 'edit')}>
        Edit
      </button>
      <button
        type="button"
        class={tabBtn(tab === 'preview')}
        onclick={() => (tab = 'preview')}
      >
        Preview
      </button>
  ...
  <!-- Scrollable body. Tab state swaps what's inside. -->
    ...
    <div class="mx-auto max-w-4xl px-4 py-5 sm:px-6 sm:py-6">
      {#if tab === 'edit'}
        ...
        <div class="mt-5">
          ...
          <textarea
            class="min-h-[320px] w-full resize-y rounded-md border border-zinc-800 bg-zinc-950 p-3 font-mono text-sm text-zinc-100 outline-none focus:border-zinc-600"
            bind:value={body}
            spellcheck="false"
          ></textarea>
        </div>
      {:else}
        <!-- Preview: same visual as the read-mode viewer so the
             user sees exactly what their commit will look like. -->
        <div class="rounded-lg border border-zinc-800 bg-zinc-900 p-4 sm:p-5">
          <h2 class="text-lg font-semibold text-zinc-50">{name.trim() || '(untitled)'}</h2>
          <div class="mt-0.5 text-xs text-zinc-500">{slug.trim() || '(no slug)'}</div>
          <p class="mt-3 text-sm text-zinc-300">{description.trim() || '(no description)'}</p>
